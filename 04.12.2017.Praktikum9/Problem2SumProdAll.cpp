...
int main()
{
    ...
    // expected: 273
    cout << sumTree(t) << endl;
    cout << operationSumOrProduct(t, 1) << endl;
}

int operationSumOrProduct(const tree<int>& t, bool doSum)
{
    if (t.empty()) {
        return doSum ? 1 : 0;
    }
    ...
    if (doSum) {
        return t.RootTree() + operationSumOrProduct(t.LeftTree(), doSum) + operationSumOrProduct(t.RightTree(), doSum);
    } else {
        return t.RootTree() * operationSumOrProduct(t.LeftTree(), doSum) * operationSumOrProduct(t.RightTree(), doSum);
    }
}
